...
# ==========================================
# 获取权重系数 (对于多分类，coef_ 形状为 [n_classes, n_features])
...
def plot_feature_importance(importances, indices, top_n=50):
    ...
    ax1.set_title(
        f"Top {top_n} Feature Importances (Logistic Regression Model)", fontsize=15)
...
def run_shap_analysis(model, X_data, feature_names=None, target_class_index=0, sample_size=100):
    ...
    # 计算 SHAP
    # model 传入的是 GridSearchCV 对象，所以要用 model.best_estimator_
    explainer = shap.LinearExplainer(model.best_estimator_, X_train_scaled)
    ...
    # 设置总标题
    fig.suptitle(
        f"SHAP Analysis for Class {target_class_index} (Logistic Regression)", fontsize=18, y=0.98)
...
# 生成列名列表
feat_names = [f"dim_{i}" for i in range(len(X_test_scaled[0]))]
