...
def get_response(prompt, temperature = 1.0, stop=None):
    completions = client.chat.completions.create(
        ...
        max_tokens=1024, # 输出内容最大可用的 token 数 (max_tokens+prompt_tokens<=model_max_tokens)
    ...
    return message.content, num_of_tokens

# 随机性 0.5
answer, num_of_tokens = get_response(prompt, 0.5)
print("第一种回答：(消耗的 token 数量：%d)\n%s\n\n" % (num_of_tokens, answer))
time.sleep(0.2)

# 随机性 1.0
answer, num_of_tokens = get_response(prompt, 1.0)
print("第二种回答：(消耗的 token 数量：%d)\n%s\n\n" % (num_of_tokens, answer))
time.sleep(0.2)

# 随机性 0.0
answer, num_of_tokens = get_response(prompt, 0.0)
print("第三种回答：(消耗的 token 数量：%d)\n%s\n\n" % (num_of_tokens, answer))
